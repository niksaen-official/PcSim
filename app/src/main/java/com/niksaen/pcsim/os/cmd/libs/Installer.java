...
public class Installer {
    ...
    public static void start(MainActivity activity, CMD cmd, String command){
        ...
        if(activity.pcParametersSave.DATA1 != null) storage.put("0",activity.pcParametersSave.DATA1);
        if(activity.pcParametersSave.DATA2 != null) storage.put("1",activity.pcParametersSave.DATA2);
        if(activity.pcParametersSave.DATA3 != null) storage.put("2",activity.pcParametersSave.DATA3);
        if(activity.pcParametersSave.DATA4 != null) storage.put("3",activity.pcParametersSave.DATA4);
        if(activity.pcParametersSave.DATA5 != null) storage.put("4",activity.pcParametersSave.DATA5);
        if(activity.pcParametersSave.DATA6 != null) storage.put("5",activity.pcParametersSave.DATA6);

        if(activity.pcParametersSave.DATA1 != null) storage.put(activity.pcParametersSave.DATA1.get("name"),activity.pcParametersSave.DATA1);
        if(activity.pcParametersSave.DATA2 != null) storage.put(activity.pcParametersSave.DATA2.get("name"),activity.pcParametersSave.DATA2);
        if(activity.pcParametersSave.DATA3 != null) storage.put(activity.pcParametersSave.DATA3.get("name"),activity.pcParametersSave.DATA3);
        if(activity.pcParametersSave.DATA4 != null) storage.put(activity.pcParametersSave.DATA4.get("name"),activity.pcParametersSave.DATA4);
        ...
        if(command.startsWith("install:")){
            ...
            if(!StringArrayWork.ArrayListToString(activity.apps).contains(program)) {
                ...
                if(storage.get(storagePos) != null) {
                    ...
                    TimerTask download = new TimerTask() {@Override public void run() {activity.runOnUiThread(() -> {
                                progress[1]++;
        ...
        if(command.startsWith("prepare.")) {
            ...
            else if (command.equals("select_main_storage")) {
                storagePos = getStorage(activity)+"";
                if (storage.get(storagePos) != null) {
                    cmd.success(activity.words.get("Drive selected"));
                } else {
                    cmd.error(activity.words.get("Drive not found"));
            ...
            else if (command.equals("select_storage_id:")) {
    ...
    private static int getStorage(MainActivity activity){
       ArrayList<HashMap<String,String>> storages = new ArrayList<>();
        storages.add(activity.pcParametersSave.DATA1);
        storages.add(activity.pcParametersSave.DATA2);
        storages.add(activity.pcParametersSave.DATA3);
        storages.add(activity.pcParametersSave.DATA4);
        storages.add(activity.pcParametersSave.DATA5);
        storages.add(activity.pcParametersSave.DATA6);
        for(int i = 0;i<storages.size();i++){
            if (storages.get(i) != null){
                if(storages.get(i).get("MainDisk").equals("true")){
                    return i;
                }
            }
        }
