...
import com.niksaen.pcsim.databinding.ProgramCheckironBinding;
...
public class DeviceManager extends Program {
    ...
    ProgramCheckironBinding binding;
    ...
    private void getParameters(){
       ...
       if (activity.pcParametersSave.CPU.get("Графическое ядро").equals("+")) {
        ...
        if(StringArrayWork.ArrayListToString(activity.apps).contains(activity.pcParametersSave.Gpu1) && activity.pcParametersSave.GPU1 != null){
            gpu1 = new String[]{
        ...
        if(StringArrayWork.ArrayListToString(activity.apps).contains(activity.pcParametersSave.Gpu2) && activity.pcParametersSave.GPU2 != null){
            gpu2 = new String[]{
